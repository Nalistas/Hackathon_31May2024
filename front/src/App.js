...
function App() {
  const [discussions, setDiscussions] = useState([
    { id: 1, title: "Lesson 1", messages: [{ id: 1, text: "Hello! How can I help you today?", sender: "bot", timestamp: new Date().toLocaleTimeString() }] },
    { id: 2, title: "Lesson 2", messages: [{ id: 1, text: "We appreciate your feedback!", sender: "bot", timestamp: new Date().toLocaleTimeString() }] },
  ]);
  ...
  const addMessage = (text) => {
    setDiscussions(discussions.map(discussion => {
      if (discussion.id === currentDiscussionId) {
        return {
          ...discussion,
          messages: [...discussion.messages, { id: discussion.messages.length + 1, text, sender: "user", timestamp: new Date().toLocaleTimeString() }],
        };
      }
      return discussion;
    }));
